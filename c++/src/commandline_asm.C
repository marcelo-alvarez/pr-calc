...
void usage(){
  ...
  if(myid==0){
    ...
    printf("\n   -o <Output base name> [default = 'map'; all output files will be prepended");
    printf("\n      with this string and in a directory called maps in the working");
    printf("\n      directory (e.g. maps/map.map, maps/map.info, etc)]");
    ...
    printf("\n   -f frequency of 21-cm map in MHz [default = 140 MHz]");
    printf("\n   -b bandwidth of 21-cm map in MHz [default = 15 MHz]");
    printf("\n\n");
  }
...
void CommandLine(int argc, char *argv[])
{
  ...
  sprintf(clParameters.BaseOut,"maps/map");
  ...
  clParameters.frequency = 140;
  clParameters.bandwidth = 10;


  while ((c = getopt (argc, argv, "hvuxdi:o:f:b:")) != -1)
    switch (c)
      {
      case 'h':
	usage();
	...
	break;
      case 'o':
	sprintf(clParameters.BaseOut,"maps/%s",optarg);
	break;
      case 'v':
	...
	break;
      case 'f':
	clParameters.frequency = atof(optarg);
	break;
      case 'b':
	clParameters.bandwidth = atof(optarg);
	break;
      case '?':
	if (optopt == 'i'){
