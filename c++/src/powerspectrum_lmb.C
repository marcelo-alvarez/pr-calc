...
void CrossPower(fftw_real *field1, fftw_real *field2, double *ps, int transform)
{
  ...
  // Initial FFT of field to k-space if transform = 1
  if(transform==1){
    rfftwnd_mpi(plan, 1, field1, NULL, FFTW_NORMAL_ORDER);
    rfftwnd_mpi(plan, 1, field2, NULL, FFTW_NORMAL_ORDER);
    NormalizeArray(field1, N, size_fftw, 1.5);
    NormalizeArray(field2, N, size_fftw, 1.5);
  }
  ...
  for(int i=0;i<Nlocal;i++){
	...
	// power = variance of mode
	float power = field1[index]*field2[index]+field1[index+1]*field2[index+1];
	lsigma += 2*power;
	if(bin>=0 && bin<nk_ps){
	  linbin[bin]++;
	  lps[bin]+= power/d3k; // variance of mode per dk^3
  ...
  // FFT of field back to real-space
  if(transform==1){
    rfftwnd_mpi(iplan, 1, field1, NULL, FFTW_NORMAL_ORDER);
    rfftwnd_mpi(iplan, 1, field2, NULL, FFTW_NORMAL_ORDER);
    NormalizeArray(field1, N, size_fftw, 1.5);
    NormalizeArray(field2, N, size_fftw, 1.5);
  }
