...
srun -n $nprocs $bin/delta2zreion parameterfiles/param.d2z -f secondrun -m $mmin -z $zeta -r $lambda -v
...
srun -n $nprocs $bin/flatskymap parameterfiles/param.fsm -i secondrun -o secondrun -v

mybanner "Output angular power spectrum to file"
/global/cscratch1/sd/ikapem/ksz-reionization/pr-calc/scripts/map2pk.py maps/secondrun.kszmap secondrun.ksz_cl.txt

# show the ksz map and output it to a pdf file
mybanner "Show ksz map"

/global/cscratch1/sd/ikapem/ksz-reionization/pr-calc/scripts/showmap.py maps/secondrun.kszmap secondrun.kszmap.pdf
