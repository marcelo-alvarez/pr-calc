...
def rad_average(image, center=None, bins=None, return_nr=False ):
    """
    Calculate the azimuthally averaged radial profile.
    ...
    if isinstance(bins, int):
        rmax = np.min(np.concatenate([center, image.shape - center]))
        bins = np.concatenate([ np.linspace(0.0, rmax / 20.0, bins / 5, endpoint=False), 
                                np.logspace(np.log10(rmax / 20.0), np.log10(rmax), 4*bins/5) ])
    ...
    wi = np.histogram(r_sorted, bins=bins, weights=i_sorted)
    wr = np.histogram(r_sorted, bins=bins, weights=r_sorted)
    w1 = np.histogram(r_sorted, bins=bins)

    # Number of pixels per radius
    rn = w1[0]
    # Construct the radial average
    rp = wi[0]/rn
    # Construct the effective radius
    re = wr[0]/rn
